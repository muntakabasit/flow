...
# Load .env — try multiple locations in priority order
_BASE = Path(__file__).resolve().parent
for _env_candidate in [
    _BASE / ".env",                                  # flow/.env  (local dev)
    _BASE.parent / ".env",                           # ofa_jack_agent/.env
    _BASE.parent / "dawt_bridge_backend" / ".env",  # legacy path
]:
    if _env_candidate.exists():
        load_dotenv(_env_candidate)
        log(f"[flow] Loaded env from {_env_candidate}")
        break

OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")
if not OPENAI_API_KEY:
    log("[flow] WARNING: OPENAI_API_KEY not set. WebSocket connections will fail.")
    log("[flow]   → Add OPENAI_API_KEY to flow/.env  or  ofa_jack_agent/.env")

# OpenAI Realtime API config
OPENAI_REALTIME_URL = "wss://api.openai.com/v1/realtime?model=gpt-4o-realtime-preview"

def get_openai_headers():
    """Build headers at connection time so runtime env vars are picked up."""
    return {
...
async def websocket_proxy(client_ws: WebSocket):
    ...
    if not OPENAI_API_KEY:
        await client_ws.send_json({
            "type": "error",
            "message": "Server misconfigured: OPENAI_API_KEY is not set. Add it to flow/.env",
        })
    ...
    try:
        ...
        openai_ws = await websockets.connect(
            OPENAI_REALTIME_URL,
            additional_headers=get_openai_headers(),
