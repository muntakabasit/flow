...
Streaming pipeline (sentence-level overlap):
  Browser mic → WebSocket → Silero VAD → mlx-whisper STT (Apple Silicon ANE)
...
MAX_SPEECH_S = 15                 # force-stop segment after 15s (conversational turns; 30s caused LLM TTFT blowout)
...
_warmup_audio = np.zeros(16000, dtype=np.float32)   # 1s of silence — primes Metal kernels
...
class StreamingVAD:
    ...
    def force_finalize(self):
        ...
        if not self.is_speaking or not self.speech_buffer:
            return []
        self.is_speaking = False
        # Trim any trailing silence already counted
...
def transcribe_segment(audio_16k, forced_source_language=None, skip_dual=False):
    ...
    needs_retry = (whisper_lang is None) and not skip_dual and (
        not supported
        or (duration_s < 2.0 and confidence < MIN_DUAL_CANDIDATE_CONF)
    )
...
async def translate_and_stream_tts(
    ...
    for attempt in range(1, OLLAMA_RETRIES + 1):
        ...
        try:
            ...
            return full_text.strip(), target_lang, llm_ms, tts_first_ms, tts_total_ms, llm_ttft_ms
...
async def handle_utterance_complete(client_ws, audio_bytes: bytes):
    if not audio_bytes:
        return
    # short segment skip (0.5s @ 16kHz, int16)
    min_bytes = int(0.5 * 16000 * 2)
    if len(audio_bytes) < min_bytes:
        log(f"[flow-local] Turn skipped: short_segment ({len(audio_bytes)} bytes < {min_bytes})")
        return

    log(f"[flow-local] Utterance complete, {len(audio_bytes)} bytes buffered")
    # Convert PCM16 -> float32 [-1,1]
    audio_16k = np.frombuffer(audio_bytes, dtype=np.int16).astype(np.float32) / 32767.0

    # Reuse existing pipeline by calling handle_speech_segment if available
    try:
        await handle_speech_segment(audio_16k)
    except Exception as e:
        log(f"[flow-local] Error in handle_utterance_complete: {e}")
...
async def websocket_handler(client_ws: WebSocket):
    ...
    try:
        while True:
            ...
            if msg_type == "speech_stopped":
            ...
            elif msg_type not in ("audio", "audio_pcm16"):
                continue
            else:
                ...
                if msg_type == "audio_pcm16" and audio_bytes is not None:
